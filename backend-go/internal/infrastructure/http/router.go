...
func registerGenerationRoutes(router gin.IRouter, handler *handlers.GenerationHandler, rateLimit gin.HandlerFunc) {
	...
	generations.GET("/:requestID/tracking", handler.Tracking)
	generations.GET("/:requestID/events", handler.Events)
	generations.POST("/:requestID/jobs/:jobID/retry", handler.RetryJob)
	generations.POST("/:requestID/retry-failed", handler.RetryFailed)
