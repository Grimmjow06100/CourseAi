...
import { ChevronDown, Lightbulb, LockKeyhole } from 'lucide-react'
    ...
    <Card>
      <CardContent>
        ...
        {exercise.payload.tasks.length ? (
          <div className="mt-5">
            <h4 className="text-sm font-semibold">{t('lesson.tasks')}</h4>
        ...
        {exercise.payload.expectedOutput ? (
          <div className="mt-5 rounded-md border border-border bg-muted p-3">
            <h4 className="mb-2 text-sm font-semibold">{t('lesson.expectedOutput')}</h4>
            <pre className="overflow-x-auto font-mono text-xs">{exercise.payload.expectedOutput}</pre>
          </div>
        ) : null}
        {exercise.payload.resources.length ? (
          <div className="mt-5">
            <h4 className="text-sm font-semibold">{t('lesson.resources')}</h4>
        ...
        {exercise.payload.hints.length ? (
          <div className="mt-5">
            ...
            {hintsOpen ? (
              <ul
                id={hintsId}
                className="mt-3 space-y-2 border-l-2 border-border pl-4 text-sm text-muted-foreground"
              >
        ...
        <div className="mt-6 border-t border-border pt-4">
          {correction ? (
            <div>
              <h4 className="mb-3 text-sm font-semibold">{t('lesson.solution')}</h4>
          ...
          {quiz.questions.map((question) => {
            ...
            return (
              <fieldset key={question.order}>
                  ...
                  <div className="mt-3 space-y-2">
                    {question.options.map((option) => {
                      ...
                      return (
                        <label
                          key={option.order}
                          className="flex cursor-pointer min-h-11 items-center gap-3 text-sm"
                        >
                          <Checkbox
                            checked={checked}
                ...
                {questionSolution ? (
                  <div className="mt-3 border-l-2 border-border bg-muted px-3 py-2 text-sm">
...
export function LessonActivities({
  ...
  return (
    <div className="mt-12 space-y-10">
      {exercises.length ? (
        <section>
          <h2 className="mb-4 text-xl font-semibold">{t('lesson.exercises')}</h2>
      ...
      {quizzes.length ? (
        <section>
          <h2 className="mb-4 text-xl font-semibold">{t('lesson.quizzes')}</h2>
          <div className="space-y-5">
            {quizzes.map((quiz) => (
